...
$token = [WinCred5]::GetPassword("Supabase CLI:supabase")
if (-not $token) {
...
function Invoke-SupabaseSQL {
    ...
    try {
        ...
        Write-Host "  [$ProjectRef] $Label OK: $($body.Substring(0, [Math]::Min(200, $body.Length)))" -ForegroundColor Green
        return $true
...
$v18 = Get-Content (Join-Path $migrDir "20260305_seed_es_dictionary_v18_missing_forms.sql") -Raw -Encoding UTF8

$TEST = "upwctgdpuckreoquofiu"
$LIVE = "jbxveulddoznswyeihda"

Write-Host "`n=== Applying to TEST ($TEST) ===" -ForegroundColor Cyan
$t1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $v18 -Token $token -Label "SEED v18 (missing forms: gniazdo×, oprawy, detektory, rury, listwa, laczniki, demontaze)"

Write-Host "`n=== Applying to LIVE ($LIVE) ===" -ForegroundColor Cyan
$l1 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $v18 -Token $token -Label "SEED v18 (missing forms)"

Write-Host ""
if ($t1 -and $l1) {
    Write-Host "ALL DONE - v18 applied to TEST + LIVE" -ForegroundColor Green
} else {
