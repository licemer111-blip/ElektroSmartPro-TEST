...
$token = [WinCred11]::GetPassword("Supabase CLI:supabase")
if (-not $token) {
...
$seedFile = Join-Path $PSScriptRoot "..\supabase\migrations\20260311_seed_es_dictionary_v11.sql"
$seedSql  = Get-Content $seedFile -Raw -Encoding UTF8

$TEST = "upwctgdpuckreoquofiu"
$LIVE = "jbxveulddoznswyeihda"

Write-Host "`n=== Applying seed v11 to TEST ($TEST) ===" -ForegroundColor Cyan
$d1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $seedSql -Token $token -Label "SEED v11 - Gniazda/Wylaczniki/Puszki (75 entries)"

Write-Host "`n=== Applying seed v11 to LIVE ($LIVE) ===" -ForegroundColor Cyan
$d2 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $seedSql -Token $token -Label "SEED v11 - Gniazda/Wylaczniki/Puszki (75 entries)"

Write-Host ""
if ($d1 -and $d2) {
    Write-Host "ALL DONE - es_dictionary v11 applied to TEST + LIVE" -ForegroundColor Green
} else {
