...
$token = [WinCredV18b]::GetPassword("Supabase CLI:supabase")
...
$sql = Get-Content (Join-Path $migrDir "20260311_seed_es_dictionary_v18b_patch.sql") -Raw -Encoding UTF8

$TEST = "upwctgdpuckreoquofiu"
$LIVE = "jbxveulddoznswyeihda"

Write-Host "`n=== TEST ($TEST) ===" -ForegroundColor Cyan
$t1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $sql -Token $token -Label "v18b patch: UPS EPO + koryta pod podloga techniczna"

Write-Host "`n=== LIVE ($LIVE) ===" -ForegroundColor Cyan
$l1 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $sql -Token $token -Label "v18b patch"

Write-Host ""
if ($t1 -and $l1) { Write-Host "ALL DONE - v18b applied TEST + LIVE" -ForegroundColor Green }
