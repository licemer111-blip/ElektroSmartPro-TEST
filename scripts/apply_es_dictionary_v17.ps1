...
$token = [WinCred4]::GetPassword("Supabase CLI:supabase")
if (-not $token) {
...
$v17 = Get-Content (Join-Path $migrDir "20260305_sync_es_dictionary_v17_14files.sql") -Raw -Encoding UTF8

$TEST = "upwctgdpuckreoquofiu"
$LIVE = "jbxveulddoznswyeihda"

Write-Host "`n=== Applying to TEST ($TEST) ===" -ForegroundColor Cyan
$t1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $v17 -Token $token -Label "SEED v17 (13 KNR categories: instalacje/trasy/oswietlenie/DALI/LAN/OZE/ziemne/pomiary/security/SSP/uziomy/zasilanie/infra)"

Write-Host "`n=== Applying to LIVE ($LIVE) ===" -ForegroundColor Cyan
$l1 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $v17 -Token $token -Label "SEED v17 (13 KNR categories)"

Write-Host ""
if ($t1 -and $l1) {
    Write-Host "ALL DONE - v17 applied to TEST + LIVE" -ForegroundColor Green
} else {
