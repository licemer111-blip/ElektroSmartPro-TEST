...
'@
$token = [WinCredReg]::GetPassword("Supabase CLI:supabase")
...
function Invoke-SupabaseSQL {
    ...
    try {
        ...
        (New-Object System.IO.StreamReader($response.GetResponseStream())).ReadToEnd() | Out-Null
        ...
        Write-Host "  [$ProjectRef] $Label ERROR: $err" -ForegroundColor Red; return $false
    }
}
$sql = Get-Content (Join-Path $PSScriptRoot "..\supabase\migrations\20260311_sync_regions_price_modifier.sql") -Raw -Encoding UTF8
$TEST = "upwctgdpuckreoquofiu"
$LIVE = "jbxveulddoznswyeihda"
Write-Host "`n=== TEST ($TEST) ===" -ForegroundColor Cyan
$t1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $sql -Token $token -Label "sync regions.price_modifier (16 voivodeships)"
Write-Host "`n=== LIVE ($LIVE) ===" -ForegroundColor Cyan
$l1 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $sql -Token $token -Label "sync regions.price_modifier (16 voivodeships)"
Write-Host ""
if ($t1 -and $l1) { Write-Host "ALL DONE - regions synced TEST + LIVE" -ForegroundColor Green }
