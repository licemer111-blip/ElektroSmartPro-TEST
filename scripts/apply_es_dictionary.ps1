...
$schemaFile = Join-Path $PSScriptRoot "..\supabase\migrations\20260301_create_es_dictionary.sql"
$seedFile   = Join-Path $PSScriptRoot "..\supabase\migrations\20260301_seed_es_dictionary.sql"
...
$s1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $schemaSql -Token $token -Label "SCHEMA (es_dictionary + pg_trgm)"
$d1 = Invoke-SupabaseSQL -ProjectRef $TEST -Sql $seedSql   -Token $token -Label "SEED   (140+ slang entries)"

Write-Host "`n=== Applying to LIVE ($LIVE) ===" -ForegroundColor Cyan
$s2 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $schemaSql -Token $token -Label "SCHEMA (es_dictionary + pg_trgm)"
$d2 = Invoke-SupabaseSQL -ProjectRef $LIVE -Sql $seedSql   -Token $token -Label "SEED   (140+ slang entries)"

Write-Host ""
if ($s1 -and $d1 -and $s2 -and $d2) {
    Write-Host "ALL DONE - es_dictionary applied to TEST + LIVE" -ForegroundColor Green
} else {
